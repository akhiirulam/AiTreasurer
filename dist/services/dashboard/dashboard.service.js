...
const dashboard_repository_1 = __importDefault(require("../../repositories/dashboard/dashboard.repository"));
class DashboardService {
    ...
    async getDashboard(userId, from, to) {
        ...
        const [transactions, accounts] = await Promise.all([
            dashboard_repository_1.default.findTransactions(userId, from, to),
            dashboard_repository_1.default.findAccounts(userId),
        ]);
        // ==================================================
        // 3. Initialize totals
        // ==================================================
        ...
        // ==================================================
        const cashBankAccounts = accounts.filter((account) => account.subCategory === "cash_and_cash_equivalents" ||
            account.subCategory === "cash_and_bank");
        const cashBankAccountIds = new Set(cashBankAccounts.map((account) => account._id.toString()));
        let cashBankBalance = 0;
        for (const transaction of transactions) {
            const amount = Number(transaction.amount || 0);
            const debitIsCashBank = transaction.debitAccountId &&
                cashBankAccountIds.has(transaction.debitAccountId.toString());
            const creditIsCashBank = transaction.creditAccountId &&
