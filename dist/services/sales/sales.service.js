...
const sales_repository_1 = __importDefault(require("../../repositories/sales/sales.repository"));
class SalesService {
    async getSales(userId, from, to) {
        // ==========================================
        // 1. VALIDATE USER ID
        // ==========================================
        ...
        // ==========================================
        const salesAccount = await sales_repository_1.default.findSalesAccount(userObjectId);
        if (!salesAccount) {
            return {
                period: {
                    from: from ?? null,
        ...
        // ==========================================
        const lines = await sales_repository_1.default.findSalesLines(userObjectId, salesAccount._id, from, to);
        // ==========================================
        ...
        // ==========================================
        const transactions = await sales_repository_1.default.findTransactions(userObjectId, transactionIds);
        ...
        const uniqueCustomerIds = Array.from(new Map(customerIds.map((id) => [id.toString(), id])).values());
        // ==========================================
        // 8. GET CUSTOMER PAYMENTS
        // ==========================================
        const payments = await sales_repository_1.default.findCustomerPayments(userObjectId, uniqueCustomerIds, from, to);
        // ==========================================
        ...
        const totalOutstanding = sales.reduce((total, sale) => total + sale.outstandingAmount, 0);
        // ==========================================
        // 13. RETURN REPORT
        // ==========================================
        return {
