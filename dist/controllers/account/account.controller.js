"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteAccount = exports.updateAccount = exports.findByName = exports.getAccount = exports.getAccounts = exports.createFromTemplate = exports.createAccount = void 0;
const account_service_1 = __importDefault(require("../../services/account/account.service"));
/**
 * Create an account manually for the authenticated user.
 */
const createAccount = async (req, res) => {
    try {
        const userId = req.userId;
        console.log("user id in accounting", userId);
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized",
            });
        }
        const { name, type, description } = req.body;
        if (!name || !type) {
            return res.status(400).json({
                success: false,
                message: "Account name and type are required",
            });
        }
        const account = await account_service_1.default.createAccount(userId, {
            name,
            type,
            description,
        });
        return res.status(201).json({
            success: true,
            message: "Account created successfully",
            data: account,
        });
    }
    catch (error) {
        console.error("Create account error:", error);
        return res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to create account",
        });
    }
};
exports.createAccount = createAccount;
/**
 * Create/get a user's account from a GLOBAL account template.
 */
const createFromTemplate = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized",
            });
        }
        const { templateName } = req.body;
        if (!templateName) {
            return res.status(400).json({
                success: false,
                message: "Template name is required",
            });
        }
        const account = await account_service_1.default.getOrCreateFromTemplate(userId, templateName);
        return res.status(201).json({
            success: true,
            message: "Account created from template successfully",
            data: account,
        });
    }
    catch (error) {
        console.error("Create account from template error:", error);
        const message = error instanceof Error
            ? error.message
            : "Failed to create account from template";
        if (message.includes("not found")) {
            return res.status(404).json({
                success: false,
                message,
            });
        }
        return res.status(500).json({
            success: false,
            message,
        });
    }
};
exports.createFromTemplate = createFromTemplate;
/**
 * Get all accounts belonging to the authenticated user.
 */
const getAccounts = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized",
            });
        }
        const accounts = await account_service_1.default.getAccounts(userId);
        return res.status(200).json({
            success: true,
            data: accounts,
        });
    }
    catch (error) {
        console.error("Get accounts error:", error);
        return res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to get accounts",
        });
    }
};
exports.getAccounts = getAccounts;
/**
 * Get one account belonging to the authenticated user.
 */
const getAccount = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized",
            });
        }
        const accountId = String(req.params.accountId);
        if (!accountId) {
            return res.status(400).json({
                success: false,
                message: "Account ID is required",
            });
        }
        const account = await account_service_1.default.getAccount(userId, accountId);
        if (!account) {
            return res.status(404).json({
                success: false,
                message: "Account not found",
            });
        }
        return res.status(200).json({
            success: true,
            data: account,
        });
    }
    catch (error) {
        console.error("Get account error:", error);
        return res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to get account",
        });
    }
};
exports.getAccount = getAccount;
/**
 * Find an account by name for the authenticated user.
 */
const findByName = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized",
            });
        }
        const { name } = req.query;
        if (typeof name !== "string" || !name.trim()) {
            return res.status(400).json({
                success: false,
                message: "Account name is required",
            });
        }
        const account = await account_service_1.default.findByName(userId, name.trim());
        if (!account) {
            return res.status(404).json({
                success: false,
                message: "Account not found",
            });
        }
        return res.status(200).json({
            success: true,
            data: account,
        });
    }
    catch (error) {
        console.error("Find account by name error:", error);
        return res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to find account",
        });
    }
};
exports.findByName = findByName;
/**
 * Update an account belonging to the authenticated user.
 */
const updateAccount = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized",
            });
        }
        const accountId = String(req.params.accountId);
        if (!accountId) {
            return res.status(400).json({
                success: false,
                message: "Account ID is required",
            });
        }
        const { name, description, isActive } = req.body;
        const account = await account_service_1.default.updateAccount(userId, accountId, {
            name,
            description,
            isActive,
        });
        if (!account) {
            return res.status(404).json({
                success: false,
                message: "Account not found",
            });
        }
        return res.status(200).json({
            success: true,
            message: "Account updated successfully",
            data: account,
        });
    }
    catch (error) {
        console.error("Update account error:", error);
        return res.status(500).json({
            success: false,
            message: error instanceof Error ? error.message : "Failed to update account",
        });
    }
};
exports.updateAccount = updateAccount;
/**
 * Delete an account belonging to the authenticated user.
 */
const deleteAccount = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({
                success: false,
                message: "Unauthorized",
            });
        }
        const accountId = String(req.params.accountId);
        if (!accountId) {
            return res.status(400).json({
                success: false,
                message: "Account ID is required",
            });
        }
        const account = await account_service_1.default.deleteAccount(userId, accountId);
        return res.status(200).json({
            success: true,
            message: "Account deleted successfully",
            data: account,
        });
    }
    catch (error) {
        console.error("Delete account error:", error);
        const message = error instanceof Error ? error.message : "Failed to delete account";
        if (message === "Account not found") {
            return res.status(404).json({
                success: false,
                message,
            });
        }
        if (message === "System accounts cannot be deleted") {
            return res.status(403).json({
                success: false,
                message,
            });
        }
        return res.status(500).json({
            success: false,
            message,
        });
    }
};
exports.deleteAccount = deleteAccount;
